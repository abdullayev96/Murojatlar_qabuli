...
def check(update, context):
    ...
    if not db_user:
        ...
        buttons = [
            [KeyboardButton(text=globals.BTN_LANG_UZ),KeyboardButton(text=globals.BTN_LANG_EN),
...
def message_handler(update, context):
    ...
    if state == 0:
        ...
        elif message == globals.BTN_USERS[db_user['lang_id']]:
            update.message.reply_text(
                text=globals.TEXT_GIVE_FEEDBACK1[db_user['lang_id']],
                parse_mode="HTML"
            )
            context.user_data["state"] = globals.STATES["user"]


        elif message == globals.BTN_INFOR[db_user['lang_id']]:
            update.message.reply_text(
                text=globals.TEXT_GIVE_FEEDBACK2[db_user['lang_id']],
                parse_mode="HTML"
            )
        ...
        update.message.reply_text(text=globals.BTN_COMMENTS_SAVE1[db_user['lang_id']],
                parse_mode="HTML")
        ...
        update.message.reply_text(text=globals.BTN_COMMENTS_SAVE[db_user['lang_id']],
                                  parse_mode="HTML")
        message = update.message.text
        db.create_test(user.id, user.username, message)
        context.bot.send_message(
            chat_id=ADMIN_ID,
            text=f"<i>{user.username}:</i>\n{message}",
            parse_mode='HTML'
        ...
        update.message.reply_text(text=globals.BTN_COMMENTS_SAVE2[db_user['lang_id']],
                                  parse_mode="HTML")
        message = update.message.text
        db.create_call(user.id, user.username, message)
        context.bot.send_message(
            chat_id=ADMIN_ID,
            text=f"<i>{user.username}:</i>\n{message}",
            parse_mode='HTML'
    ...
    #     date = last_news["posted_at"].split(' ')[0]
        ...
        # elif path1 == settings.MEDIA_ROOT:
        #     newPath = path1.replace(os.sep, '/')
        #     update.message.reply_photo(
        #         photo=open(f'{newPath}/{last_news["image"]}', "rb"),
        #         caption=f"""<b>{last_news[f'heading_{globals.LANGUAGE_CODE[db_user["lang_id"]]}']}</b>\n{last_news[f'text_{globals.LANGUAGE_CODE[db_user["lang_id"]]}']}\n<i>{date}</i>""",
        ...
        #     )
        # else:
        #     update.message.reply_text(text="Bunday ID raqam mavjud emas")
        # else:
        #     path1 = settings.MEDIA_ROOT
        #     newPath = path1.replace(os.sep, '/')
        #     update.message.reply_photo(
        ...
        #         document=open(f'{newPath}/{last_news["file"]}', "rb"),
        #         caption=f"""<b>{last_news[f'heading_{globals.LANGUAGE_CODE[db_user["lang_id"]]}']}</b>\n{last_news[f'text_{globals.LANGUAGE_CODE[db_user["lang_id"]]}']}\n<i>{date}</i>""",
        #         parse_mode="HTML"
        #     )
